...
class Mapper(SLAMParameters):
    ...
    def mapping(self):
        ...
        while True:
            ...
            # mapping_cams = len(self.mapping_cams)
            if len(self.mapping_cams)>0:
                ...
                print(f"[Mapper] Iteration {self.train_iter} - Loss: {loss.item():.4f} - RGB Loss: {loss_rgb.item():.4f} - Depth Loss: {loss_d.item():.4f}")
        ...
        self.calc_2d_metric()
        print(f"[Mapper] Final number of Gaussians in the map: {self.gaussians.get_xyz.shape[0]}")

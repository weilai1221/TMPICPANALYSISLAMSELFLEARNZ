...
def clean_points(points):
    points = points[~np.isnan(points).any(axis=1)]  # remove NaN
    points = points[~np.isinf(points).any(axis=1)]  # remove Inf
    return points
...
def main():
	...
	for i, filename in enumerate(filenames):
		...
		if i == 0:
			...
			# delta = np.identity(4)
			reg.set_input_target(points)
			reg.set_input_source(points)  # Set both target and source to same points
			delta = reg.align()           # Trigger covariances computation safely
			reg.set_input_target(points)  # Reset correct target
		else:

			print(f"Frame {i}: source points shape = {points.shape}")
			if points.size == 0:
				print("Warning: empty point cloud at frame", i)

			if np.isnan(points).any():
				print(f"Warning: NaN detected in point cloud at frame {i}")

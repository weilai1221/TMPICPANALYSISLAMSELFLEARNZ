...
#pragma omp parallel for num_threads(num_threads_) firstprivate(k_indices, k_sq_dists) schedule(guided, 8)
    for (int i = 0; i < input_->size(); i++)
    {
      ...
      if (!search_target_)
      {
        std::cerr << "[ERROR] search_target_ is nullptr!" << std::endl;
        exit(-1);
      }
      if (!target_)
      {
        std::cerr << "[ERROR] target_ is nullptr!" << std::endl;
        exit(-1);
      }
      if (target_->empty())
      {
        std::cerr << "[ERROR] target_ pointcloud is EMPTY!" << std::endl;
        exit(-1);
      }
      std::cout << "[INFO] update_correspondences: target size = " << target_->size() << std::endl;
      */
      ...
      /*
      // Before determinant() check, regularize RCR
      const double regularization_epsilon = 1e-6;
      RCR.topLeftCorner<3, 3>() += regularization_epsilon * Eigen::Matrix3d::Identity();
      */
